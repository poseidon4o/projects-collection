...
int Engine::getPixelHeight() const {
	return this->pixelSize.y;
}

int Engine::getPixelWidth() const {
...
void Engine::_setPixelSize() {
	...
	for(int c = 25; c >= this->pixelSize.x; --c) {
		for(int r = 25; r >= this->pixelSize.y; --r) {
	...
	pFont->dwFontSize.X = this->pixelSize.x = bX;
	pFont->dwFontSize.Y = this->pixelSize.y = bY;
	SetCurrentConsoleFontEx(hConsole,0,pFont);
	delete pFont;
}
...
void Engine::_resizeConsole(int rows,int cols) const{
	...
	sprintf(buf,"mode %d, %d",cols,rows);
	system(buf);
	delete[] buf;
